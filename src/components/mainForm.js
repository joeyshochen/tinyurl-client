...
export const MainForm = React.memo(
	({ setSubmissionCompleted, srcUrl, setSrcUrl, alias, setAlias }) => {
		const [srcUrlDirty, setSrcUrlDirty] = React.useState(false);
		const [srcUrlValidationMsg, setSrcUrlValidationMsg] =
			React.useState("");
		...
		const formSubmitOnClick = React.useCallback(async () => {
			...
			if (srcUrl && alias) {
				...
				const result = await axios
					.get(`${tinyUrlServer}/api/getAlias/${alias}`)
					.catch((error) => {
						console.log(error);
					});
				...
				if (result?.data !== "Not found") {
					...
					setAliasValidationMsg(
						"This alias is already in use. Please choose another."
					);
					return;
				}

				axios
					.post(`${tinyUrlServer}/api/create-tinyurl`, {
						srcUrl,
						alias,
					})
					.catch((error) => {
		...
		return (
			<div className="container" style={{ maxWidth: 500 }}>
				<h2>TinyURL</h2>
				<p className="lead">
					Create an alias that can be used for your URL to shorten it.
				</p>
				<Form
					validated={!srcUrlDirty && !aliasDirty && srcUrl && alias}
				>
